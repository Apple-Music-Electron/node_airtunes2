...
UDPServers.prototype.bind = function(host) {
  ...
  case BOUND:
    process.nextTick(function() {
      self.emit('ports', null, this.control, this.timing, this.event);
    });
    return;
  ...
  // Find open ports
  var to_bind = [this.control, this.timing, this.event];
  ...
  async.whilst(
    ...
    function(err) {
      if(err) {
        ...
        self.status = BOUND;
        self.emit('ports', null, self.control, self.timing, self.event);
      }
    }
  );
}
