...
function AirTunesDevice(host, audioOut, options, mode = 0, txt = "") {
  ...
  try {
    if (!(process.platform == "darwin")) {
      // MacOSX arm64 segfaults on airtunes.node
      this.bindings = require("../build/Release/airtunes.node");
  ...
  if (this.statusflags != []) {
    let PasswordRequired =
      this.statusflags[this.statusflags.length - 1 - 7] == "1";
    let PinRequired = this.statusflags[this.statusflags.length - 1 - 3] == "1";
    let OneTimePairingRequired =
      this.statusflags[this.statusflags.length - 1 - 9] == "1";
    console.log(
      "needPss",
      PasswordRequired,
      PinRequired,
    ...
    this.transient =
      !(PasswordRequired || PinRequired || OneTimePairingRequired) ?? true;
  ...
  if (
    (k[0] ?? "").includes("AppleTV3,1") ||
    (k[0] ?? "").includes("AirReceiver3,1") ||
    (k[0] ?? "").includes("AirRecever3,1") ||
    (k[0] ?? "").includes("Shairport")
  ) {
    this.alacEncoding = true;
    this.airplay2 = false;
  }
  ...
  let manufacturer = this.txt.filter((u) =>
...
AirTunesDevice.prototype.setTrackInfo = function (
  name,
  artist,
  album,
  callback
...
function makeAirTunesPacket(
  ...
  var alac =
    alacEncoding || credentials
      ? pcmToALAC(encoder, packet.pcm, bindings, bindingsok)
      : pcmParse(encoder, packet.pcm);
  ...
  if (requireEncryption) {
    if (bindingsok) {
      if (bindings == null) {
        bindings = require("../build/Release/airtunes.node");
      }
...
function pcmToALAC(encoder, pcmData, bindings, bindingsok) {
  ...
  if (bindingsok == true) {
    var alacSize = bindings.encodeALAC(
      encoder,
      pcmData,
      alacData,
      pcmData.length
    ...
    let bsize = 352,
      frames = 352; // Set these to whatever they should be
    ...
    while (count--) {
      ...
      p[pindex++] =
        ((i & 0x007f0000) >>> 15) | ((input[iindex] & 0x00008000) >> 15); // b16--b15, b7 from in[pindex]
    }
...
function encryptAES(alacData, alacSize) {
  ...
  const isv = Buffer.from([
    0x78, 0xf4, 0x41, 0x2c, 0x8d, 0x17, 0x37, 0x90, 0x2b, 0x15, 0xa6, 0xb3,
    0xee, 0x77, 0x0d, 0x67,
  ]);
  const aes_key = Buffer.from([
    0x14, 0x49, 0x7d, 0xcc, 0x98, 0xe1, 0x37, 0xa8, 0x55, 0xc1, 0x45, 0x5a,
    0x6b, 0xc0, 0xc9, 0x79,
  ]);
...
function makeRTPHeader(packet) {
  ...
  if (packet.seq === 0) header.writeUInt16BE(0x80e0, 0);
  else header.writeUInt16BE(0x8060, 0);
