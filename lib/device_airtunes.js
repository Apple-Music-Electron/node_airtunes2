...
function AirTunesDevice(host, audioOut, options, mode = 0, txt = "") {
  ...
  try{
      if (!(process.platform == 'darwin' && process.arch == 'arm64')) { // MacOSX arm64 segfaults on airtunes.node
  ...
  let d = this.txt.filter((u) => String(u).startsWith('ft='))
  let featureset = d[0] ? d[0].substring(3).split(',') : []
  this.features = [... featureset.length > 0 ? parseInt(featureset[0]).toString(2).split('') : [], ... featureset.length > 1 ? parseInt(featureset[1]).toString(2).split('') : []]
  ...
  if (this.features.length > 0){
    this.redundancyAllowed = (this.features[this.features.length - 1 - 11] == '1')
    console.log('redundancy', this.redundancyAllowed)
  }
  ...
  this.audioPacketRawHistory = new BufferWithNames(10000); 
  this.encoder = [];
...
AirTunesDevice.prototype.relayAudio = function() {
  ...
  this.audioCallback = function(packet) {
    let oldpackets = null
    if (self.redundancyAllowed) {
      if (self.audioPacketHistory.buffer.length > 7) {
        oldpackets = (self.audioPacketRawHistory.buffer.slice(-7).reverse());
      }
    }
      var [airTunes, rawaudio] = makeAirTunesPacket(packet, self.encoder, self.requireEncryption, self.alacEncoding, self.credentials, self.bindings, self.bindingsok, oldpackets);
      self.sendAirTunesPacket(airTunes); 
    self.audioPacketRawHistory.add(packet.seq, rawaudio);
...
function makeAirTunesPacket(packet, encoder, requireEncryption, alacEncoding = true, credentials = null, bindings, bindingsok, oldpackets) {
  ...
  let rawaudio = credentials ? packet.pcm : alac;
  let header = makeRTPHeader(packet,oldpackets ? true : false);     
  if (oldpackets) {
   let audiopackets = oldpackets.map((u) => u[1]);
   // we gonna assume 352 frames per packet here
   let adheader = new Buffer(4 * oldpackets.length);
   adheader.writeUInt16BE(0x80e1, 0)
  }       
...
function makeRTPHeader(packet, redundancy) {
  var header = new Buffer(RTP_HEADER_SIZE);
 
  if(packet.seq === 0) {
    if (redundancy) {header.writeUInt16BE(0x80e1, 0)} else {header.writeUInt16BE(0x80e0, 0)}}
  else {
    if (redundancy) {header.writeUInt16BE(0x80e1, 0)} else {header.writeUInt16BE(0x80e0, 0)}}
