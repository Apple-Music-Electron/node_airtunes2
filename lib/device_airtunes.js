...
function makeAirTunesPacket(packet, encoder, requireEncryption, alacEncoding = true, credentials = null, bindings, bindingsok) {
  ...
  var alac = (alacEncoding ) ? pcmToALAC(encoder, packet.pcm, self.bindings, self.bindingsok) : pcmParse(encoder, packet.pcm);
  var airTunes = new Buffer(alac.length + RTP_HEADER_SIZE + 16 + 8);
  ...
  if (credentials) {
    alac = credentials.encryptAudio(packet.pcm,header.slice(4,12))
    airTunes = new Buffer(RTP_HEADER_SIZE + alac.length);
...
function pcmToALAC(encoder, pcmData, bindings, bindingsok) {
  ...
  console.log("bindingsok",bindingsok)
  if (bindingsok == true) {
