...
Client.prototype.sendNextRequest = function(di) {
  ...
  case PAIR_VERIFY_HAP_1:
    ...
    let hap1kp = curve25519.generateKeyPair()
    this.verifyPrivate = hap1kp.private
    this.verifyPublic = hap1kp.public
  ...
  case AUTH_SETUP:
    ...
    finalbuffer = Buffer.concat([Buffer.from(request, 'utf-8'),
    ...
    if (this.airplay2){
      try {
      finalbuffer =  this.credentials.encrypt(finalbuffer)
      } catch (e){
...
Client.prototype.processData = function(blob, rawData) {
  ...
  switch(this.status) {
    ...
    case PAIR_VERIFY_HAP_1:
      ...
      let sharedSecret = curve25519.sharedKey(this.verifyPrivate,sessionPublicKey)
    ...
    case SETUP_AP2_1:
      ...
      this.timingPort = sa1_bplist['timingPort']
