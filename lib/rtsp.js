...
Client.prototype.sendNextRequest = function(di) {
  ...
  case PAIR_SETUP_1:
    ...
    // request += 'User-Agent: AirPlay/381.13\r\n'
  ...
  switch(this.status) {
    ...
    case PAIR_SETUP_1:
      ...
      if (databuf1[tlv.Tag.BackOff]) {
        ...
        let seconds = Buffer.from(backOff).readIntBE(0, backOff.byteLength);
        
        console.log("You've attempt to pair too recently. Try again in " + (seconds) + " seconds.");
        
      }
      console.log(this._atv_salt)
      this._hap_genkey = hexString2ArrayBuffer('60975527035CF2AD1989806F0407210BC81EDC04E2762A56AFD529DDDA2D4393')
      this.srp = new SrpClient(SRP.params.hap,Buffer.from(this._atv_salt),Buffer.from("Pair-Setup",'ascii'),Buffer.from(this.password.toString(),'ascii'),this._hap_genkey)
