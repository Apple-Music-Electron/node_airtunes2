...
;
  ...
  case SETUP_AP2_2:
    ...
    let setap2 =  bplistCreator(
      {streams: [{audioFormat: 262144, // PCM/44100/16/2
          ...
          streamConnectionID: this.announceId + 1
          }]});
  ...
  case RECORD:
    ...
    if (this.airplay2 != null && this.credentials != null) {
      ...
      let rct = this.credentials.encrypt(Buffer.from(request, 'ascii'));
...
Client.prototype.sendControlSync = function(seq, dev) {
  ...
  console.log('dev.host',dev.host)
  this.controlsocket.send(packet, 0, packet.length, dev.controlPort, dev.host);
}