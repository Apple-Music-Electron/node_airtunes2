...
function Client(volume, password, audioOut, options) {
  ...
  this.dacpId = nu.randomHex(8).toString().toUpperCase();
  this.session = null;
  this.timeout = null;
  this.volume = volume;
...
;
  ...
  case SETUP_AP2_1:
    ...
    // if (this.hosts.indexOf(rinfo.address) < 0) return;
      ...
      self.timingsocket.send(
        ...
        self.timingDestPort ? self.timingDestPort : rinfo.port, rinfo.address
      );
      if (this.debug) console.log('timing reply', self.timingDestPort ? self.timingDestPort : rinfo.port, rinfo.port, ts1, ts2, ntpTime);
    });
  ...
  case SETDAAP:
    let daapenc = false;
    if (this.borkedshp) {daapenc = true} 
...
Client.prototype.daapEncode = function(field, value, enc) {
  ...
  buf.write(field, 0, field.length, enc ? 'utf-8' : "ascii");
  buf.writeUInt32BE(value.length, field.length);
  buf.write(value, field.length + 4, value.length, enc ? 'utf-8' : "ascii");
  return buf;
};
...
Client.prototype.processData = function(blob, rawData) {
  console.log('Receiving request:',rtsp_methods[this.status+1]);
  ...
  if(response.code !== 200) {
    if (this.status != SETVOLUME && this.status != SETPEERS && this.status != FLUSH && this.status != RECORD && this.status != GETVOLUME) {
      if ([PAIR_VERIFY_1,
        PAIR_VERIFY_2,
        AUTH_SETUP,
        PAIR_PIN_START,
  ...
  switch(this.status) {
    ...
    case SETUP_AP2_1:
      ...
      // this.timingsocket.removeAllListeners('message')
      // var self = this;
      // this.timingsocket.on('message', function(msg, rinfo) {
      ...
      //     self.timingDestPort ? self.timingDestPort : rinfo.port, rinfo.address
      //   );
      //   if (this.debug) console.log('timing reply 2', self.timingDestPort ? self.timingDestPort : rinfo.port, rinfo.port, ts1, ts2, ntpTime);
      // });
      // this.timingsocket.bind(this.timingPort, this.socket.address().address);
