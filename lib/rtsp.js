...
function Client(volume, password, audioOut, mode) {
...
Client.prototype.startHandshake = function(udpServers, host, port) {
  ...
  this.socket = net.connect(port, host, async function() {
    ...
    if (self.password != null && self.mode == 0) {
      ...
      await atv.authSimple(self.password);
...
Client.prototype.sendNextRequest = function(di) {
  ...
  case PAIR_VERIFY_2:
    ...
    request = ''
    // const verifier = ATVAuthenticator.verifier('3c0591f41d1236c9ce5078cd6fcd42f71f374b8b6dff33fea825366f1c34f828');
  ...
  break;
  // case PAIR_VERIFY_2:
  //   request += this.makeHead("POST","/pair-verify", di);
  // break;
  case OPTIONS:
