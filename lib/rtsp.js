...
;
  ...
  case INFO:
    ...
    let sdada = number.UInt16toBufferBE(enct1x.byteLength)
    console.log(sdada)
    console.log(request)
    this.socket.write(Buffer.concat([Buffer.concat([sdada.slice(1), sdada.slice(0,1)]),Buffer.from(enct1x)]))
    console.log("yas")
    request = ''
    //console.log(request);  
  break;
  case SETUP_AP2_1:
    ...
    let enct1 = this.credentials.encrypt(Buffer.concat([Buffer.from(request, 'utf-8'),setap1]));
    let sdada2 = number.UInt16toBufferBE(enct1.byteLength);
    this.socket.write(Buffer.concat([Buffer.concat([sdada2.slice(1), sdada2.slice(0,1)]),enct1]));
    request = ''
  break;
  case SETUP_AP2_2:
    this.announceId = nu.randomInt(8);
    ...
    let setap2 =  bplistCreator(
      {streams: {audioFormat: 262144,
          ...
          latencyMax: 88200,
          latencyMin: 11025,
          shk: Buffer.from(this.verifier_hap_1.sharedSecret),
          spf: 352,
          supportsDynamicStreamID: true,
          type: 96}});
    request += 'Content-Length: ' + Buffer.byteLength(setap2) + '\r\n\r\n';
    console.log(Buffer.concat([Buffer.from(request, 'utf-8'),setap2]).toString())
    let enct2 = this.credentials.encrypt(Buffer.concat([Buffer.from(request, 'utf-8'),setap2]));
    let sdada3 = number.UInt16toBufferBE(enct2.byteLength);
    this.socket.write(Buffer.concat([Buffer.concat([sdada3.slice(1), sdada3.slice(0,1)]),enct2]));
    request = ''      
  break;  
  case RECORD:
