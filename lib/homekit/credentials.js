    ...
    */
    ...
    encrypt(message) {
        ...
        while (offset < total) {
            ...
            let [s1ct , s1tag] = encryption_1.default.encryptAndSeal(message,s1length_bytes,Buffer.concat([Buffer.from([0x00,0x00,0x00,0x00]),struct.pack("Q", this.encryptCount)]),this.writeKey)
    ...
    decrypt(message) {
        ...
        // decipher.final();
        let decrypted = encryption_1.default.verifyAndDecrypt(cipherText,hmac,message.slice(0,2),Buffer.concat([Buffer.from([0x00,0x00,0x00,0x00]),struct.pack("Q", this.decryptCount)]),this.readKey)
        this.decryptCount += 1
