...
function computePoly1305(cipherText, AAD, nonce, key) {
    ...
    let polyKey = null;
    let computed_hmac = null
    if (nonce.byteLength == 12) {
        // console.log(JSON.stringify(Object.keys(sodium_1.api)))
        polyKey = sodium_1.api.crypto_stream_chacha20_ietf(32, nonce, key);
        computed_hmac = sodium_1.api.crypto_onetimeauth(msg, polyKey);
        polyKey.fill(0);
    } else {
     polyKey = sodium_1.api.crypto_stream_chacha20(32, nonce, key);
     computed_hmac = sodium_1.api.crypto_onetimeauth(msg, polyKey);
    polyKey.fill(0);}
...
function verifyAndDecrypt(cipherText, mac, AAD, nonce, key) {
    if (nonce.byteLength == 12) {
        ...
        if (matches === 0) {
            return sodium_1.api
                .crypto_stream_chacha20_poly1305_xor_ic(cipherText, nonce, 1, key);
        }
    } else {
        const matches = sodium_1.api.crypto_verify_16(mac, computePoly1305(cipherText, AAD, nonce, key));
        if (matches === 0) {
...
function encryptAndSeal(plainText, AAD, nonce, key) {
        ...
        let hmac = null;
    if (nonce.byteLength == 12) {
        cipherText = sodium_1.api
        .crypto_stream_chacha20_poly1305_xor_ic(plainText, nonce, 1, key);
        hmac = computePoly1305IETF(cipherText, AAD, nonce, key);
    } else {
        cipherText = sodium_1.api
        .crypto_stream_chacha20_xor_ic(plainText, nonce, 1, key);
        hmac = computePoly1305(cipherText, AAD, nonce, key);
    }
