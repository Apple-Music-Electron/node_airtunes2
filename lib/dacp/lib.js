...
function DACPServer() {
  ...
  this.cache = {
    ...
    cafe: 0,
    cave: 0,
    canp: "00002e74000000000000000000002fea",
    cann: "",
    cana: "",
    canl: "",
    cang: "Pop",
...
DACPServer.prototype.start = async function () {
  this.browser.scan();
  let port = await getPort(14243);
  this.startHTTPServer(port);
  this.browser.broadcast(port);
  this.rpc.start();
  ...
  this.rpc.on("nowplayingItem", (item) => {
    ...
    if (this.previousplaystatus != null && this.previousplaystatus[1] != null){
      ...
      for (let item of ["caps", "cann", "cana", "canl"]) {
...
DACPServer.prototype.startHTTPServer = function (port) {
  // express server
  const app = express();
  ...
  app.get("/ctrl-int/1/getproperty", (req, res) => {
   // console.log("[DACP] property");
    ...
    for (property of properties) {
      switch (property) {
        ...
        case "dacp.playingtime":
          ...
          break;
        case "dmcp.volume":
          if (this.previousvol != this.rpc.volume) {
            daap_frame.push(["cmvo", this.rpc.volume]);
            this.previousvol = this.rpc.volume;
          } else {
            ...
            this.volumeInterval = setInterval(() => {
              if (this.previousvol == this.rpc.volume) {
                ...
                let array = ['cmgt', [["mstt", 200], ...inside]]
    ...
    var data = daap_encode2(["cmgt", [["mstt", 200], ...daap_frame]]);
  ...
  app.get("/ctrl-int/1/setproperty", (req, res) => {
    ...
    for (let key in properties) {
      switch (key) {
        case "dmcp.volume":
          this.rpc.setvol(properties[key]);
          this.cache["cmvo"] = this.rpc.volume;
          break;
        case "dacp.shufflestate":
          this.rpc.setShuffle(properties[key]);
          break;
        case "dacp.repeatstate":
          this.rpc.setRepeat(properties[key]);
          break;
        case "dacp.playingtime":
          this.rpc.seekTo(properties[key]);
          break;
      }
    }
    res.set({
  ...
  app.get("/ctrl-int/1/playstatusupdate", async (req, res) => {
    ...
    //   return;
    // }
    let next_rev = parseInt(rev_number) + 1;
